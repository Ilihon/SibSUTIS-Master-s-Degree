...
BigUnsignedInABase::BigUnsignedInABase(const std::string &s, Base base) {
	...
	for (digitNum = 0; digitNum < len; digitNum++) {
		...
		if (theSymbol >= '0' && theSymbol <= '9')
			...
			throw std::exception("BigUnsignedInABase(std::string, Base): Bad symbol in input.  Only 0-9, A-Z, a-z are accepted.");

		if (blk[digitNum] >= base)
			throw std::exception("BigUnsignedInABase::BigUnsignedInABase(const Digit *, Index, Base): A digit is too large for the specified base");
	}
	zapLeadingZeros();
}
