...
int main(int argc, char **argv)
{
    ...
    double max_res_time[REPEAT] = {0};
    double arg_allred = 123.456;
    double res_allred = 0.0;
	...
	// Wait for barrier time
		...
		MPI_Allreduce(&arg_allred, &res_allred, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		double local_time_stop = MPI_Wtime();
		double result_time = local_time_stop - local_time;

        	MPI_Barrier(MPI_COMM_WORLD);

        	MPI_Allreduce(&result_time, &max_res_time[repeat_counter], 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
    ...
    for(repeat_counter = 0; repeat_counter < REPEAT; repeat_counter++)
    {
       	average_value += max_res_time[repeat_counter];
    }

    average_value /= REPEAT;

    printf("Allreduce time: %f\n", average_value);
